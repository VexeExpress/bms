    ...
    return (
        ...
        >
            <div className="mb-2 flex items-center justify-between">
                ...
                {ticket.ticketPhone && (
                    <div className="rounded border border-[#3d3d3d] px-2 ml-2 py-[1px] text-[16px] font-semibold text-black">
                        {ticket.ticketPhone}
                    </div>
                )}
            </div>
                ...
                {ticket.paymentType && (
                    <div className="mt-2 flex items-center justify-between">
                        <div className="flex items-center">
                            ...
                            <span className="text-[13px] font-medium text-black">{ticket.ticketPrice}</span>
                        </div>
...
const TicketMap = ({ tripId }: TicketMapProps) => {
    ...
    const handleSelectTicket = (ticket: TicketData) => {
        ...
        const hasValidTicket = selectedTickets.some((t: TicketData) => t.ticketPhone);
        ...
        if (!ticket.ticketPhone) {
            ...
            const updatedTickets = selectedTickets.some(t => t.ticketPhone === ticket.ticketPhone)
    ...
    const handleDeleteChip = (ticketId: number) => {
        setSelectedTickets((prevSelectedTickets) => {
            const newSelectedTickets = prevSelectedTickets.filter((t) => t.id !== ticketId);
...
const BookingTicket = ({
    ...
    const handleUpdate = async () => {
        ...
        try {
            const updatedTickets = await updateTicket(ticketsToUpdate.map(ticket => ticket.ticketId), data);
