...
import { Box, Button, Card, Grid, Grid2, MenuItem, Modal, Select, TextField, Typography } from "@mui/material";
                    ...
                    rowSeats.push(
                        <div key={`${seatKey}`} className={style.seat_card}>
                            ...
                            <div className={style.toggle_container}>
                                <span className={`font-rounded ${style.toggle_labe}`}>Kích hoạt</span>
                ...
                <form onSubmit={handleSubmit(onSubmitForm)} className="mt-5">
                    <Grid container spacing={4}>
                        <Grid item xs={12} sm={6} md={6}>
                            <label htmlFor="seatMapName" className="block font-rounded font-semibold text-black">Tên sơ đồ</label>
                        ...
                        <Grid item xs={12} sm={6} md={2}>
                            <label htmlFor="floor" className="block font-rounded font-semibold text-black">Số tầng</label>
                            <Select
                                id="floor"
                                {...register('floor')}
                                className="w-full mt-1 border border-gray-300 rounded font-rounded"
                                size="small"
                                onChange={(e) => setSelectedFloor(Number(e.target.value))}
                            >
                                {Array.from({ length: 3 }, (_, index) => (
                                    <MenuItem key={index + 1} value={index + 1} className="font-rounded">
                                        {index + 1} tầng
                                    </MenuItem>
                                ))}
                        ...
                        <Grid item xs={12} sm={6} md={2}>
                            <label htmlFor="row" className="block font-rounded font-semibold text-black">Số hàng</label>
                            <Select
                                id="row"
                        ...
                        <Grid item xs={12} sm={6} md={2}>
                            <label htmlFor="column" className="block font-rounded font-semibold text-black">Số cột</label>
                            <Select
                                id="column"
                                {...register('column')}
                                className="w-full mt-1 border border-gray-300 rounded font-rounded"
